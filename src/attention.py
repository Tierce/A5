...
class CausalSelfAttention(nn.Module):
    ...
    # hs = d/h
        ...
        B, T, C = x.size() #batch size, block size, total dimensionality
...
class SynthesizerAttention(nn.Module):
    def __init__(self, config):
        ...
        self.w1 = nn.Linear(config.n_embd, config.n_embd) # (B, nh, T, hs)
        self.w2 = nn.Parameter(torch.zeros(config.n_embd // config.n_head, #(d/h,l)
            config.block_size-1))
        self.b2 = nn.Parameter(torch.zeros(config.block_size-1)) #(B,l)
    ...
    def forward(self, x, layer_past=None):
        ...
        w1 = self.w1(x).view(B, T, self.n_head, C // self.n_head).transpose(1, 2) # (B, nh, l, d/h)
        w2 = self.w2 
        b2 = self.b2
