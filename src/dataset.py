...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if len(document)>6:
            masklength = round((len(document)/4)+random.sample([-1,0,1],1)[0])
        else:
            masklength = round((len(document)/4))
        if len(document)-masklength == 0:
            mask_start_id = random.randint(0,len(document)-masklength)
        else:mask_start_id = random.randint(0,len(document)-masklength-1)
        mask_end_id = mask_start_id + masklength
        
        x = document[:mask_start_id] + self.MASK_CHAR + document[mask_end_id:] + self.MASK_CHAR + document[mask_start_id:mask_end_id]
        x = x + self.PAD_CHAR*(self.block_size - len(x))
